...
ox-hugo will pass the "noexport" tag to hugo, which breaks some of my blogging
workflows.  The expected behaviour is to filter it out (since org-export should
not export that heading).
#+begin_src emacs-lisp
(defun my/hugo--tag-processing-fn-remove-noexport (tag-list _info)
  (seq-filter (lambda (x) (not (equal x "noexport")))
              tag-list))
(add-to-list 'org-hugo-tag-processing-functions #'my/hugo--tag-processing-fn-remove-noexport)
#+end_src
...
(map!
 ...
 :desc "Advance and Archive" "SPC" #'el-secretario-notmuch-advance-and-archive
 :desc "Mark message as done" "d" (cmd! (notmuch-show-archive-thread)
                                        (el-secretario-next-item))

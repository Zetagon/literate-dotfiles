...
#+end_src
#+begin_src emacs-lisp
(defun my/org-refile-to-query (query &optional map-fun)
...
"
  (let ((entry-list (--> query
                         (org-ql-select (org-agenda-files) it
                           :action (lambda ()
                         ...
                         (if map-fun
                           ...
                           it)
                         (mapcar (lambda (x)
                                   (cl-destructuring-bind (file line-text line-num tmp-plist) x
                                       (let ((plist (nth 1 tmp-plist)))
                                         (-as-> plist
                                                that
                                                (plist-put that :file-name file)
                                                (plist-put that :raw-value
                                                           (concat (number-to-string line-num)
                                                                   ": "
                                                                   line-text)))
                                         plist)))
                                 it))))
    ...
    (-as->
     (--> (completing-read "Refile target" (mapcar (lambda (x) (plist-get x :raw-value)) entry-list))
          (-filter (lambda (x) (string-equal (plist-get x :raw-value) it)) entry-list)
          car)
     that

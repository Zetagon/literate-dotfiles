...
#+include: ~/.doom.d/shell.org
#+include: ~/.doom.d/email.org
#+include: ~/.doom.d/version-control.org
* Setup
...
(package! elgantt :pin "be49f119bf09ff4ebe98c0ca3f1907191e3603d6"
  :recipe (:repo "https://github.com/legalnonsense/elgantt/"))
(package! org-pandoc-import
  :recipe (:local-repo "~/Downloads/org-pandoc-import"))
(package! brutalist-theme
  :recipe (:repo "https://github.com/aparkerdavid/brutalist-theme/")
  :pin "a291f9a520e5d5692d645a05880092a54cd30b2f")

(package! org-fc
  :pin "1d9d9e9d356b0c64ef8fc2710e5c63506e839049"
  :recipe (:repo "https://git.sr.ht/~l3kn/org-fc"
...
(package! el-secretario
  ...
  ;; :recipe (:repo "https://github.com/Zetagon/el-secretario")
  ;; :pin "b59941ea4b4f91cd4f5e4f00189d549149cfcbb2"
  )
(package! ox-hugo)
#+END_SRC
...
(setq doom-theme 'brutalist)
(use-package! org-fc
  :after org
  :config
  (setq org-fc-directories '("~/Documents/notes/fc/"))
...
Nameless mode is handy to hack in namespaces in elisp
...
#+begin_src emacs-lisp :tangle ~/.doom.d/packages.el
(package! nameless :pin "a3a1ce3ec0c5724bcbfe553d831bd4f6b3fe863a")
#+end_src
...
(my/define-double-key my/org-roam-insert #'org-roam-insert)
...
#+begin_src emacs-lisp
(after! embark
  (defvar my/embark-org-element-target-finders '((link . my/embark-target-org-link))
    "An (ELEMENT-TYPE . TARGET-FINDER) alist to determine which org elementshas its own embark actions.

ELEMENT-TYPE is a symbol that is the first element of the result of
`org-element-context'. TARGET-FINDER is a function that works as
specified in `embark-target-finders', with the exception that it
takes an argument that is the the second element of the
returned value of `org-element-context'.")
...
ACTION-TYPE is a symbol and should be a key in embark-keymap-alist.")
  ...
  (mapc (lambda (x)
          (push (cons x 'org-ref-cite-link) my/embark-org-link-type-alist))
        org-ref-cite-types)

  (mapc (lambda (x)
          (push (cons x 'org-ref-ref-link) my/embark-org-link-type-alist))
        org-ref-ref-types)

  (defun my/embark-target-org-context ()
    "Target the org element at point.

This is an abstract function. What elements are recognized as
targets is determined by `my/embark-org-element-target-finders'.
If the element at point is not specified in
`my/embark-org-element-target-finders' return nil.
"
    (when (eq major-mode
            'org-mode)
        (when-let ((context (org-element-context)))
          (when-let ((finder (alist-get (car context) my/embark-org-element-target-finders)))
            (funcall finder (cl-second context))))))

  (add-to-list 'embark-target-finders #'my/embark-target-org-context)

  (defun my/embark-target-org-link (link)
    "Target org link at point.

If a matching entry is found in `my/embark-org-link-type-alist'
use that as action type. Otherwise the action type is 'url.
"
  ...
  (embark-define-keymap embark-org-ref-cite-map
    "Embark keymap for org-ref cite: links")
  (map! :map embark-org-ref-cite-map
        :desc "Open citation at point" "o" (lambda (&optional _) (funcall org-ref-open-citation-at-point))
        :desc "Open pdf" "p" (lambda (&optional _) (funcall org-ref-open-pdf-function))
        :desc "Open notes" "n" #'org-ref-open-notes-at-point
        :desc "Open URL" "u" #'org-ref-open-url-at-point
        ...
        :desc "Copy key to clipboard" "k" (λ! (progn (kill-new (car (org-ref-get-bibtex-key-and-file)))))
        :desc "Copy formated entry" "f" (λ! (kill-new (org-ref-format-entry (org-ref-get-bibtex-key-under-cursor))))
        :desc "Email entry" "e" (λ! (kill-new (save-excursion (org-ref-open-citation-at-point) (org-ref-email-bibtex-entry))))
        :desc "Insert cite" "i" (lambda (&optional _) (funcall org-ref-insert-cite-function))
        :desc "Change cite type" "h" (lambda (&optional _)  (call-interactively #'org-ref-change-cite-type)))

  (embark-define-keymap embark-org-ref-ref-map
    "Embark keymap for org-ref ref: links")
  (map! :map embark-org-ref-ref-map
        :desc "Insert ref" "i" (lambda () (funcall org-ref-insert-ref-function))
        :desc "Change ref type" "h" (lambda (&optional _) (call-interactively #'org-ref-change-ref-type)))

  (add-to-list 'embark-keymap-alist '(org-ref-cite-link . embark-org-ref-cite-map))
  (add-to-list 'embark-keymap-alist '(org-ref-ref-link . embark-org-ref-ref-map)))
#+end_src
...
(map!
 ...
 :desc "Schedule" "s" (cmd!
                       (el-secretario-space-schedule-and-reset nil "-365d")
                       (el-secretario/next-item))
 :desc "Begin writing" "b" (cmd!
                            (el-secretario-space--reset)
                            (el-secretario-tasks-begin-task))
 :desc  "Delete visible" "D" (cmd! (delete-region (point-min) (point-max)))
 :desc  "Quit" "q" (cmd! (el-secretario-end-sesion)))
#+end_src

#+begin_src emacs-lisp
(defun my/begin-writing ()
  (interactive)
  (el-secretario-start-session (el-secretario-org-make-source
                                '(todo)
                                '("~/org/writing-inbox.org")
...
(after! bibtex-completion
  (setq bibtex-completion-bibliography "~/Documents/notes/Zotero_articles.bib"
        ...
        bibtex-completion-library-path "~/Documents/notes/pdfs"
  ...
  (when (featurep! :completion selectrum)
    ;; I have embark-act, so click enter needs to only go to the entry
    (setq org-ref-cite-onclick-function (lambda (_) (org-ref-open-citation-at-point))))
...
(map!
 ...
 (:when (featurep! :completion ivy)
  :n "nh" #'ivy-bibtex)
 (:when (featurep! :completion selectrum)
  :n "nh" #'bibtex-actions-insert-citation))
#+END_SRC

There seems to be issues with the embark actions in the bibtex-completion package. The which-key descriptions are from other keybindings. Mapping them manually seems to fix the issue.

#+begin_src emacs-lisp
(after! bibtex-completion
  (map! :map bibtex-actions-map
        :desc "Add pdf attachment" "t" 'bibtex-actions-add-pdf-attachment
        :desc "Add pdf to library" "a" 'bibtex-actions-add-pdf-to-library
        :desc "Insert bibtex entry" "b" 'bibtex-actions-insert-bibtex
        :desc "Insert citation" "c" 'bibtex-actions-insert-citation
        :desc "Insert key" "k" 'bibtex-actions-insert-key
        :desc "Insert reference" "f" 'bibtex-actions-insert-reference
        :desc "Open" "o" 'bibtex-actions-open
        :desc "Open entry" "e" 'bibtex-actions-open-entry
        :desc "Open notes" "l" 'bibtex-actions-open-link
        :desc "Open notes" "n" 'bibtex-actions-open-notes
        :desc "Open pdf" "p" 'bibtex-actions-open-pdf
        :desc "Refresh" "r" 'bibtex-actions-refresh))
#+end_src
* Task management
:PROPERTIES:
...
(defun my/today ()
  ...
  (let ((org-agenda-custom-commands
         '(("l" "Today"
            ((tags-todo "active" ((org-agenda-overriding-header "Active tasks")))
...
#+BEGIN_SRC emacs-lisp
(defvar my/secretary-notmuch-map (make-sparse-keymap))
(map!
 :map my/secretary-notmuch-map
 :desc "next" "n" (cmd!
                   (notmuch-show-tag-all '("-notdone"))
                   (org-capture nil "m")
                   (el-secretario/next-item))

 :desc "Quit" "q" #'el-secretario-end-sesion
 :desc "Mark message as done" "d" (cmd! (notmuch-show-tag-all '("-notdone"))
                                        (el-secretario/next-item))

 :desc "Capture" "c" (cmd!
                      (org-capture nil "f"))

 :desc "Next message" "]" #'notmuch-show-next-message
 :desc "Previous message" "[" #'notmuch-show-previous-message)
#+END_SRC
...
(map! :map my/el-secretario-org-map
      ...
      :desc "Activate task" "a" (cmd!
                                  (save-excursion
                                    (goto-char (point-min))
                                    (el-secretario-org-add-tag "active")
                                    (el-secretario-space--reset)
                                    (org-schedule "+7d")))
      ...
      :desc "Link" "l" #'link-hint-open-link
      :desc "Schedule" "s" (cmd!
                             (call-interactively #'el-secretario-space-schedule-and-reset)
                             (el-secretario/next-item))
...
(defun my/usb-backup ()
  ...
  (when (y-or-n-p "Have you plugged in the USB?")
    (shell-command "~/.doom.d/usb-backup.sh")))
#+end_src
**** El Secretario
#+BEGIN_SRC emacs-lisp
(after! el-secretario
...
(defun my/dailyreview-secretary ()
  (list
   (el-secretario-function-source :func (lambda ()
                                          (browse-url "https://etidning.dn.se/")
   ...
   (el-secretario-function-source :func
                                  (lambda ()
                                    ...
                                    (el-secretario-activate-keymap)))
   (el-secretario-notmuch-make-source   "to:leo@relevant-information.com AND tag:notdone AND NOT tag:deleted NOT tag:gmail/Inbox" #'my/secretary-notmuch-map)
   (el-secretario-notmuch-make-source   "tag:lists AND tag:notdone AND NOT tag:deleted NOT tag:gmail/Inbox" #'my/secretary-notmuch-map)
   (el-secretario-notmuch-make-source   "tag:notdone AND NOT tag:deleted NOT tag:gmail/Inbox" #'my/secretary-notmuch-map)
   ...
   (el-secretario-org-make-source '(and (or (and (todo "TODO")
                                                 (not (org-entry-blocked-p))
                                                 (not (tags-inherited "active"))
                                  ...
                                  :next-item-hook' (lambda ()
                                                     (el-secretario-org-remove-tag "active"))
                                  :keymap 'my/el-secretario-org-map)

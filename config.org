...
#+begin_src emacs-lisp
(defhydra my/el-secretario-writing-hydra ()
  ("n" (progn
         (el-secretario-space-reschedule)
         (el-secretario-next-item) ) "next (and spaced rep)" :exit t)
  ("l" link-hint-open-link "Link")
  ("s" (progn
         (el-secretario-space-schedule-and-reset nil "-365d")
         (el-secretario-next-item)) "Schedule")
  ("b" (progn
         (el-secretario-space--reset)
         (el-secretario-tasks-begin-task)) "Begin writing" :exit t)
...
(defun my/begin-writing ()
  ...
  (el-secretario-start-session (list
                                (el-secretario-org-make-source
                                 ...
                                 :hydra #'my/el-secretario-writing-hydra/body
                                 :shuffle-p t
...
(defhydra my/el-secretario-org-hydra ()
  ...
  ("n" (progn
         (el-secretario-space-reschedule)
         (el-secretario-next-item) ) "next (and spaced rep)" :exit t)
  ("RET" (el-secretario-next-item)  "next" :exit t)
  ("r" (let ((org-reverse-note-order nil))
         (org-refile)
         (el-secretario-next-item)) "Refile" :exit t)
  ...
  (defun my/dailyreview-secretary ()
    (list
     ...
     (el-secretario-org-make-source '(and (todo "WAITING")
                                    ...
                                    '("~/org/orgzly/Todo.org"
                                      "~/org/orgzly/Projects.org")
                                    :hydra #'my/el-secretario-org-hydra/body)

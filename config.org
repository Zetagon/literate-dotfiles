...
(defun my/push-dailies-to-weekly ()
  ...
  (interactive)
  (org-ql-query
    :select (λ!  (org-set-tags (cons "weeklyreview" (org-get-tags nil local))))
    :from (current-buffer)
    :where '(and (todo)
                 (not (org-entry-blocked-p))
    ...
    (pcase-dolist (`(,file-name ,file-time ,entries) table-list)
      (pcase-dolist (`(,level ,headline ,tags-list ,ts ,time ,props) entries)
        (dolist (tag tags-list)
          (when (-contains? tracked-tags tag)
...
"
      ...
      (insert "plot ")
      (dolist (p plots)
        (insert (format "$data%d" i) " with circles linecolor rgb \"" (plist-get p :color) "\" title \"" (plist-get p :title) "\" ,\\\n")
        (setq i (+ 1 i)))
      (delete-char -2)
      (save-buffer)
